# -*- coding: utf-8 -*-
"""
Created on Sat Feb 26 22:24:57 2022
...
#function the get the values for a attribute
...
# fucntion to run the ID3 algorithm on tictactoe dataset
def tictactoe_id3():
    
    #loading the raw mushroom data
    raw_data = np.genfromtxt('./tictactoe.data', missing_values=0, skip_header=0, delimiter=',', dtype=int)
    
    #splitting the raw data to train and test sets
    train, test = train_test_split(raw_data, test_size=0.2)
    
    #creating the train labels and attributes
    ytrn = train[:,9]
    Xtrn = train[:,:9]
    
    #creating the test labels and attributes
    ytst = test[:,9]
    Xtst = test[:,:9]
    ...
    for i in [1,3,5]:
        ...
        render_dot_file(dot_str, './tictactoe_c_'+str(i))
        ...
        print("tictactoe id3 tree depth:"+str(i)+" matrix")
...
# function to run the sklearn DecisionTreeClassifier on the tictactoe dataset
def tictactoe_sktree():
    
    #loading the raw tictactoe data
    raw_data = np.genfromtxt('./tictactoe.data', missing_values=0, skip_header=0, delimiter=',', dtype=int)
    ...
    for i in [1,3,5]:
        ...
        tree.export_graphviz(decision_tree, out_file = './tictactoe_d_' + str(i), class_names = ['0', '1'] , filled = True)
        ...
        y_training = decision_tree.predict(Xtrn)
        trn_err = compute_error(ytrn, y_training)
        ...
        #get the confusion matrix
        cm = confusion_matrix(ytst,y_pred)
        print("tictactoe scikit tree depth:"+str(i)+" matrix")
        print('Training Error = {0:4.2f}%.'.format(trn_err * 100))
        print('Test Error = {0:4.2f}%.'.format(tst_err * 100))
        print(cm)
...
if __name__ == '__main__':
    ...
    #part e
    # running id3 on tictactoe dataset
    tictactoe_id3()
    
    #part e
    # running sklearn DecisionTreeClassifier on tictactoe dataset
    tictactoe_sktree()